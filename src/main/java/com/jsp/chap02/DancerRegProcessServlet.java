...
public class DancerRegProcessServlet extends HttpServlet {
    ...
    //사용자의 입력값 가져오기
    //가져온 입력값을 토대로 Dancer 객체 생성 -> repository의 save()가 대신 하는중
    //정보를 데이터베이스에 저장 -> repository의 save()가 대신 하는중
    //결과를 화면으로 응답
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //POST 요청에만 호출됩니다 -> GET 요청에는 반응하지 X

        //POST 방식에서 전달된 한글 데이터 깨짐 방지 메서드
        ...
        //checkbox같이 여러 값을 전달받은 경우에는 getParameterValues 사용 -> String 배열로 리턴
        ...
        System.out.println("genresArray = " + Arrays.toString(genresArray)); //체크박스라 여러값 출력
         */

        // 얻어온 입력값을 save()에게 전달 -> 객체 생성 및 Map에 저장까지 일괄처리(save 참조)
        DancerRepository.save(name,crewName,danceLevel,genresArray);
        //save 전달 받은 값을 객체로 포장, 리스트를 포장, genre에
        //출력
        //dancerMap에 put
